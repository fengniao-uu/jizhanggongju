...
async function api(path, opts = {}) {
  ...
  // 无论什么环境，都在第二个位置放本地 5000 兜底：
  //   - 生产部署（域名）：第一个 API_BASE（相对路径）就会成功，直接 return，不会多花任何时间 / 任何请求
  //   - 本地开发 8081 / 5188 / file://：第一个失败 → 试 5000 Flask → 自动兜底成功
  ...
  for (let i = 0; i < tryBases.length; i++) {
    ...
    let res = null;
    try {
      res = await fetch(base + path, mergedOpts);
    } catch (e) {
    ...
    const httpFail = (typeof res.status === "number") && (res.status < 200 || res.status >= 400);
    const codeFail = (typeof json.code === "number") && (json.code < 0 || json.code >= 400);
    const shouldFallback = (i < tryBases.length - 1) && (httpFail || codeFail);
    if (shouldFallback) {
      failCodes.push({ base: base || "/", http: res.status, code: json.code, msg: (json.msg || "").substring(0, 60) });
      console.warn("[api] 上游失败（base=" + (base || "/") + ", HTTP=" + res.status + ", code=" + json.code + "）" + path + " → fallback 到 " + tryBases[i+1]);
      continue;
    }
...
async function apiForm(path, method, formData) {
  ...
  if (token) headers["Authorization"] = "Bearer " + token;
  try {
    const res = await fetch(API_BASE + path, { method, headers, body: formData, credentials: "omit" });
